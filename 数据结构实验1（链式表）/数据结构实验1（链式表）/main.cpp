...
//显示：显示当前所有学生信息记录；
void show_student(LinkList<student>&a) {
	if (a.IsEmpty())
		cout << "没有学生" << endl;
	else {
...
void set_massage(LinkList<student> &a) {
	system("cls");
	int sign = 0;
	cout << "1.在尾部插入" << endl;
	cout << "2.输入插入位置" << endl;
...
//删除：删除指定位置的学生信息记录；
void delete_him(LinkList<student> &a) {
	int sign = 0;
	student s;
	cout << "输入位置：(在1-" << a.GetLength() << "之间)" << endl;
	cin >> sign;
...
//更新：更新指定位置的学生信息记录；
void change(LinkList<student> &a) {
	system("cls");
	int sign = 0;
	student s;
	cout << "输入位置：(在1-" << a.GetLength() << "之间)" << endl;
...
//统计：统计表中学生人数。
void sum_student(LinkList<student> &a) {
	system("cls");
	cout << "学生人数为：" << a.GetLength() << endl;
}
//排序：按照学号或者绩点进行排序
void sort(LinkList<student> &a) {
	system("cls");
	if (a.GetLength() < 2) {
		cout << "人数过少，无法排序" << endl;
		return;
...
//清空：清空表中所有记录
void clear_all(LinkList<student> &a) {
